...
class GraphClassfier(torch.nn.Module):
    ...
    def __init__(self, layer_nums, actions, num_feat, num_label, drop_outs, multi_label=False, 
        ...
        super(GraphClassfier, self).__init__()     
    ...
    def build_hidden_layers(self, actions, batch_normal, drop_outs, layer_nums, num_feat, num_label, state_num=5):
        ...
        ### readout layers
        self.readout = actions[-1]
            
        self.last_fc = nn.Linear(out_channels, self.num_label)
    ...
    def forward(self, x, edge_index_all, batch):
        ...
        if self.readout == "sum_pool":
            output = global_add_pool(output, batch)
        elif self.readout == "max_pool":
